...
button.addEventListener("click", function () {
    ...
    chrome.storage.sync.set({ 'lastPageLinkManga': lastPageLink }, function () {
    });
    ...
    lastPageTitle = lastPageTitle_array[0] + 'Kapitel ' + (parseInt(number) + 1) + lastPageTitle_array[1].substring(number.length);

    chrome.storage.sync.set({ 'lastPageTitleManga': lastPageTitle }, function () {
    });

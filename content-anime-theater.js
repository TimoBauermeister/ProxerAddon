...
s.src = browser.runtime.getURL("content-anime-donatecall.js");
s.onload = function () {};
  ...
  if (result && result["theaterMode"]) {
    ...
    setTimeout(() => {
      ...
      while (table.querySelector(".menu") != undefined) {
        ...
        // next chapter
        if (
          elem.textContent.includes("Nächste") &&
          !elem.textContent.includes("Watchlist")
        ) {
        ...
        if (elem.getAttribute("title") == "reminder_next") {
          elem.setAttribute(
            "class",
            elem.getAttribute("class") + " reminderNext"
          );
        ...
        if (elem.getAttribute("title") == "reminder_this") {
          elem.setAttribute(
            "class",
            elem.getAttribute("class") + " reminderThis"
          );
          elem.innerHTML = "&#128278;";
        }
      ...
      // text
      let textMiddle =
        document.getElementById("wContainer").childNodes[3].childNodes[0];
